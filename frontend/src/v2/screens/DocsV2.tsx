...
const ENDPOINTS = [
  { method: 'POST', path: '/api/v1/upload', desc: '简单上传', color: '#178a5a' },
  { method: 'POST', path: '/api/v1/upload/init', desc: '初始化分块上传', color: '#178a5a' },
  { method: 'POST', path: '/api/v1/upload/{id}/sign-part', desc: '签名上传分块', color: '#178a5a' },
  { method: 'POST', path: '/api/v1/upload/{id}/complete', desc: '完成分块上传', color: '#178a5a' },
  { method: 'GET', path: '/api/v1/shares/{code}', desc: '读取分享信息', color: '#3f7bb3' },
  { method: 'DELETE', path: '/api/v1/upload/{id}', desc: '取消上传', color: '#c2402f' },
];
...
export function DocsV2() {
  ...
  return <div data-r="pad" style={{maxWidth:920,width:'100%',margin:'0 auto',padding:'32px 24px 40px',flex:1}}>
    ...
    <h1 style={{fontSize:32,fontWeight:700,letterSpacing:'-.02em',color:'var(--tx)'}}>API 文档</h1>
    ...
    <div style={{marginTop:24,display:'flex',flexDirection:'column',gap:10}}>
      {ENDPOINTS.map(e=><div key={e.method+e.path} style={{border:'1px solid var(--ln)',borderRadius:12,background:'var(--pn)',padding:'14px 16px',display:'flex',alignItems:'center',gap:14,flexWrap:'wrap'}}>
        ...
        <span style={{fontSize:13,color:'var(--tx3)',marginLeft:'auto'}}>{e.desc}</span>
      </div>)}
    </div>
