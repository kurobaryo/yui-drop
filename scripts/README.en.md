...
[中文 (default)](./README.md) · English · [日本語](./README.ja.md)
...
| `install-yuidrop.sh` | One-shot installer — symlinks the CLI into `/usr/local/bin` and writes `/etc/yuidrop.conf`. |
...
1. **Symlink** `scripts/yuidrop.sh` to `/usr/local/bin/yuidrop` (via `ln -sf`)
   and `chmod +x` the target. Because it's a symlink rather than a copy, the
   CLI on `PATH` is automatically refreshed every time `yuidrop update` runs
   `git pull` — no need to re-run the installer after each release.
...
The health endpoint is `/api/health`; the CLI auto-discovers the host-mapped
port via `docker port <container> 8000` so installations that publish 8000 to
a non-default host port (e.g. `18823` behind nginx-proxy-manager) just work.
Override with `YUIDROP_HEALTH_URL=https://…/api/health` if you want to probe
through your reverse proxy instead.

## Commands

```text
yuidrop update [--force|-f]  Pull origin/main (fast-forward only), rebuild image,
                             run alembic migrations, hit the health endpoint.
                             Refuses to run if the working tree is dirty.
                             --force / -f rebuilds even when git is already up
                             to date — useful when the running container is
                             stale (e.g. you pulled manually earlier, or the
                             previous rebuild was aborted).
...
# Force a rebuild even when git is already up to date
# (e.g. the previous deploy was aborted mid-build)
